<div class="col-lg-3 order-lg-1 order-2">
    <!-- shop-sidebar-wrap start -->
    <div class="shop-sidebar-wrap">


        <!-- shop-sidebar start -->
        <div class="shop-sidebar mb-30">
            <h4 class="title">CATEGORIES</h4>
            <ul>
              @foreach($kategori as $kat)
              @php
                $jlhProdukPerKategori = DB::table('tbl_produk') -> where('kategori', $kat -> kd_kategori) -> count();
              @endphp
                <li><a href="#!">{{ $kat -> nama_kategori }} <span>({{ $jlhProdukPerKategori }})</span></a></li>
              @endforeach
            </ul>
        </div>
        <!-- shop-sidebar end -->


        <!-- shop-sidebar start -->
        <div class="sidbar-product shop-sidebar mb-30">
            <h4 class="title">best product</h4>
            <!-- sidbar-product-inner start -->
            <div class="sidbar-product-inner">
                <div class="product-image">
                    <a href="product-details.html"><img src="https://s3-id-jkt-1.kilatstorage.id/ebunga/product/main-product/EBUNGA4280.jpg" alt=""></a>
                </div>
                <div class="product-content text-left">
                    <h3><a href="product-details.html">Products Name 003</a></h3>
                    <div class="price-box">
                        <span class="old-price">11.00</span>
                        <span class="new-price">10.00</span>
                    </div>
                </div>
            </div>
            <!-- sidbar-product-inner end -->
        </div>
        <!-- shop-sidebar end -->



        <!-- shop-sidebar start -->
        <div class="shop-sidebar">
            <h4 class="title">Hot Tags</h4>
            <div class="sidebar-tag">
                <a href="#">Imlek</a>
                <a href="#">Valentine</a>
                <a href="#">Love</a>
                <a href="#">Birthday</a>
                <a href="#">Souvenir</a>
                <a href="#">Graduation</a>
                <a href="#">Fresh Flower</a>
            </div>
        </div>
        <!-- shop-sidebar end -->
    </div>
    <!-- shop-sidebar-wrap end -->
</div>
